...
/**
 * Given a set of hashes either loads from from local network parameters storage or requests them from the other peer. Downloaded
...
class FetchNetworkParametersFlow(requests: Set<SecureHash>,
    ...
    override fun load(txid: SecureHash): SignedParametersByHash? {
        return serviceHub.networkParametersStorage.readSignedParameters(txid)?.let { SignedParametersByHash(it) }
    }

    // TODO check types
    override fun convert(wire: SignedDataWithCert<NetworkParameters>): SignedParametersByHash = SignedParametersByHash(wire)

    // TODO check what with nullability
